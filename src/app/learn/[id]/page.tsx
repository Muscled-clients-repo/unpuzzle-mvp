...
export default function PublicLessonPage() {
  ...
  // For public lessons, treat the lessonId as both courseId and videoId
  ...
  // Load lessons for public lessons (always standalone)
  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true)
      // Public lessons always load from the lessons store
      if (lessons.length === 0) {
        console.log('📚 Loading lessons for public lesson:', videoId)
        await loadLessons()
      }
    ...
    loadData()
  }, [videoId, loadLessons, lessons.length])
