...
export interface Message {
  ...
  contextData?: {  // For chat messages with video context
    transcript: string
    startTime: number
    endTime: number
  }
...
export interface Action {
  type: 'AGENT_BUTTON_CLICKED' | 'VIDEO_MANUALLY_PAUSED' | 'VIDEO_PLAYED' | 'ACCEPT_AGENT' | 'REJECT_AGENT' | 'QUIZ_ANSWER_SELECTED' | 'REFLECTION_SUBMITTED' | 'REFLECTION_TYPE_CHOSEN' | 'REFLECTION_CANCELLED' | 'SET_IN_POINT' | 'SET_OUT_POINT' | 'CLEAR_SEGMENT' | 'UPDATE_SEGMENT' | 'SEND_SEGMENT_TO_CHAT' | 'RECORDING_STARTED' | 'RECORDING_PAUSED' | 'RECORDING_RESUMED' | 'RECORDING_STOPPED' | 'AI_GENERATION_STARTED' | 'AI_GENERATION_CHUNK' | 'AI_GENERATION_COMPLETE' | 'AI_GENERATION_ERROR' | 'ADD_CHAT_MESSAGE'
  payload: any
}