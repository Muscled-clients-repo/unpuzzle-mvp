-- Migration script to move data from fragmented tables to unified conversation system
...
BEGIN
  ...
  FROM (
    -- From daily notes (student conversations)
    SELECT
  ...
  ON CONFLICT (student_id, instructor_id) DO NOTHING; -- Avoid duplicates if re-run
